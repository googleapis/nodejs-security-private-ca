...
function main(name) {
  // [START privateca_schedule_delete_certificate_authority_sample]
  /**
  ...
  async function scheduleDeleteCertificateAuthority() {
    // Construct request
    const request = {
      name,
    };
    ...
    // Run request
    const [operation] =
      await privatecaClient.scheduleDeleteCertificateAuthority(request);
    const [response] = await operation.promise();
    console.log(response);
  }

  scheduleDeleteCertificateAuthority();
  // [END privateca_schedule_delete_certificate_authority_sample]
}
